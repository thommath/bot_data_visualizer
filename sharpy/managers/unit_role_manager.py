...
class UnitRoleManager(ManagerBase):
    ...
    def _defenders_from(
        ...
        for unit in role_units.sorted_by_distance_to(position):
            ...
            if not (self.unit_values.can_shoot_ground(unit) or unit.type_id in [UnitTypeId.BANELING, UnitTypeId.LURKER]) and enough_air_power and not enough_ground_power:
                # Don't pull any more units that can't actually shoot the targets
                continue
