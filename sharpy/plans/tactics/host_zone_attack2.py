...
class AttackStatus(Enum):
    ...
    ProtectingExpansion = (
        4,
    )  # NYI, holding enemy expansion and preventing enemy expansions
    Retreat = (10,)  # Prefers to escape without fighting
    Withdraw = (11,)  # Fights any enemies while escaping


class HostPlanZoneAttack(ActBase):
    ...
    def __init__(self, start_attack_power: float = 20):
        assert isinstance(start_attack_power, float) or isinstance(
            start_attack_power, int
        )
        super().__init__()
        self.retreat_multiplier = HostPlanZoneAttack.RETREAT_POWER_PERCENTAGE
    ...
    async def execute(self) -> bool:
        ...
        if self.status == AttackStatus.Attacking:
            ...
            for unit in attacking_units:
                ...
                at_gather_point = (
                    pos.distance_to(self.knowledge.gather_point)
    ...
    def handle_attack(self, target):
        ...
        # Execute
        # Check for Swarm Host cooldowns and retreat if we don't have locusts
        landed_locusts = self.get_count(UnitTypeId.LOCUSTMP)
        flying_locusts = self.get_count(UnitTypeId.LOCUSTMPFLYING)
        host_tags = [host.tag for host in self.cache.own(UnitTypeId.SWARMHOSTMP)]
        ...
        if host_tags:
            off_cd = [
                ...
                if self.knowledge.cooldown_manager.available_dict[tag]
            ]
            if (
                not landed_locusts
                and not flying_locusts
                and len(off_cd) < 0.5 * len(host_tags)
            ):
                back_up = True

        if back_up:
            back_target = self.pather.find_low_inside_walk(
                front_runner.position, target, 5
            )
            self.combat.execute(back_target, MoveType.PanicRetreat)
        else:
    ...
    def _should_retreat(
        self, fight_center: Point2, already_attacking: Units
    ) -> AttackStatus:
        enemy_local_units: Units = self.knowledge.known_enemy_units.closer_than(
            HostPlanZoneAttack.DISTANCE_TO_INCLUDE, fight_center
        )

        if self.knowledge.enemy_worker_type is not None:
            enemy_local_units = enemy_local_units.exclude_type(
                self.knowledge.enemy_worker_type
            )
    ...
    def _get_target(self) -> Optional[Point2]:
        ...
        proxy_buildings = self.knowledge.known_enemy_structures.closer_than(
            70, our_main
        )
        ...
        enemy_zones = list(
            filter(lambda z: z.is_enemys, self.knowledge.expansion_zones)
        )
